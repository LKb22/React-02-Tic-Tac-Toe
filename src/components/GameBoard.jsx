...
/* Checking for a Match STEP 3: Move the INITIAL_GAME_BOARD to the App component along with the gameBoard state:
...
export default function GameBoard({ onSelectSquare, board }) {
  ...
  */
	...
	// GameBoard STEP 3: useState - We need to manage the state of the gameboard so that, when a player clicks a button, we can change and update the UI of that button with the players symbol without having to reload the page. The default value of useState / state value is the INITIAL_GAME_BOARD state. Use destructuring as usual.
	// const [gameBoard, setGameBoard] = useState(INITIAL_GAME_BOARD);

	// GameBoard STEP 4: Create the function that will handle the button click event. As the pointer function, this function handles the logic. Essentially, in this case, it calls the update state function ('setGameBoard') to A: get the current state value, it then B: updates/ changes that value, and then it C: passes it back as an arguement, using return, to the update state function to re-render the UI with the new state value.
  ...
  */
	...
	return (
		<ol id="game-board">
			{/* GameBoard STEP 2: Use .map to dynamically render the game board cells. Map takes a function as an input that will be called automatically for every item in the array. Each child array in the parent array represents a 'row', which shoud be a list item. React requires every item iterated with .map to have a unique key so that it can be identified. We can simply use the index as the key (this is not always best practice because the indexes can change/items can move places, but this won't happen in this case). Now, in each of the three arrays, represented by 'row' in our iteration, we need map again to iterate over their values and output the individual cells as buttons. Each button, in the initial state, has no value since it hasn't been clicked yet. When it's clicked, it will receive that player's symbol. Therefore, we can use {playerSymbol} to represent each button dynamically.

      GameBoard STEP 5: Add onClick to the buttons to handle the click event, passing the handleSelectSquare as the pointer function that handles the logic of the click event. Use the trick of passing an anonymous function as the value to onClick, with the pointer inside of that function so that we can add arguements and control how/when it's called.

      GameBoard STEP 6: Of course, we now want to use the 'gameBoard state snapshot' here instead of the ititial gameboard, so that the board being rendered is the one with the current state vaule.
